    ...
    <Deck theme={customTheme} template={template} transition={{
      ...
      <Slide>
        <FlexBox height="100%">
          ...
          <Notes>
            Сегодня наша встреча будет состоять из двух с половиной частей.

            В первой половинке я представлюсь и немного скажу про себя.

            Дальше расскажу общие вещи про то как люди строят компиляторы, прям в очень общих чертах, поскольку тема
            реально необъятная.

            Ну и наконец, расскажу историю из жизни, а именно - зачем, почему и главное как я делал не самый
            востребованный на рынке инструмент.
          </Notes>
        </FlexBox>
      </Slide>
      ...
      <Slide>
        <FlexBox height="100%" justifyContent='space-around'>
          ...
          <Notes>
            Итак. Я родился в Сибири, выходец из НГТУ с факультета вычислительной техники, отучился на специальности "мастер на все руки",
      ...
      <Slide>
        <FlexBox height="100%" justifyContent='space-around'>
          ...
          <Notes>
            Важнее то, что я играю в маджонг, смотрю японские мультики, глажу котиков и пытаюсь делать мир лучше
      ...
      <Slide>
        <FlexBox height="100%" justifyContent='space-around'>
          ...
          <Notes>
            Еще я успел поработать в 2гис, wrike, Вконтакте, подрасти с середнячкового мидла до должности архитектора,
      ...
      <Slide>
        ...
        <Notes>
          Немного о наверное самой дезориентирующей штуке, которую мне когда-либо доводилось имплементировать. Что если
          необходимо убрать из вывода лишний код? Наверное стоит начать с того, что такое лишний код.

          Ну например - вот есть у вас реакт-компонент с обработчиками событий типа onclick. Очевидно, что эти обработчики
          исполняются только на клиенте после гидрации, на сервере в них смысла никакого нет.

          Окей, допустим мы уберем рендеринг пропертей типа onClick или onЧтото. Но этого недостаточно - останутся хвосты
          в виде неиспользуемых функций и переменных. Убираем эти функции и переменные - могут остаться другие неиспользуемые
          функции и переменные, и далее по спирали.
        </Notes>
      </Slide>
      ...
      <Slide>
        ...
        <Notes>
          Если вкратце (а не вкратце у нас не получится, потому что элиминация кода это тема для целого доклада, а то и не одного),
          то мы можем здесь следующее:
          1) Собрать из всего кода идентификаторы - а это будут и функции, и переменные.
          2) Установить связи между идентификаторами - если идентификатор 1 используется при формировании значения идентификатора 2,
          значит идентификатор 2 зависит от идентификатора 1.
          3) Задать некоторый очень ограниченный набор правил, согласно которым некий идентификатор будет считаться заведомо
          _используемым_. В качестве таких правил можно например взять наличие ключевого слова export, или использование
          идентификатора в console.log() при выводе.
          4) Пройти по получившемуся графу и пометить идентификаторы используемыми, если от них зависят идентификаторы, которые
          уже помечены как используемые
          5) Профит!
        </Notes>
      </Slide>
      ...
      <Slide>
        ...
        <Notes>
          Насколько мне известно, новый транспилятор использовали в мобильной версии VK и паре подпроектов еще до моего
          ухода из компании. Верю, что пацаны в состоянии довести дело до ума.

          Вообще же инструмент лежит в открытом доступе в гитхабе VK, ссылку давать не буду потому что реклама :)
          Но если интересно посмотреть и пощупать - прошу в личку, ссылками поделюсь охотно.
        </Notes>
      </Slide>
